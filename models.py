...
class BERT_RC(nn.Module):
  ...
  def forward(self, **inputs):
    ...
    entity_marker = torch.tensor(inputs['entity_marker'], dtype = torch.int64).unsqueeze(dim = 0).to(next(self.parameters()).device)
    hidden_states = self.encoder(input_ids = input_ids, attention_mask = attention_mask)
    outs = torch.cat([hidden_states.last_hidden_state[torch.arange(len(hidden_states)), entity_marker[:,0]],
                      hidden_states.last_hidden_state[torch.arange(len(hidden_states)), entity_marker[:,1]]], dim = 1)
